...
- **P5 全量验收 + 用户反馈闭环**：k6 端到端压测（10k 硬门禁 + 50k 留痕）、安全回归（slowapi 限流 + 多 worker 一致 + 启动 fail-fast）、LangSmith 评测消费、用户反馈闭环（匿名收藏/不喜欢 + 防刷）、部署须知（start.sh / 盐轮换 / 50k 留痕）、扩展点文档。*已完成（2026-08-29 验收通过：240 测试全绿 + 6 条 Playwright 冒烟 + k6 10k 门禁通过 + 50k 基线留痕），实施计划与验收结果见 [docs/P5_PLAN.md](P5_PLAN.md)。*
- **P6 部署上线**：Docker Compose 全栈（app + MySQL + Redis + Caddy 自动 HTTPS）、就绪门控（service_healthy + entrypoint 等待/迁移）、可信代理 IP（BEHIND_PROXY + FORWARDED_ALLOW_IPS + XFF 右到左解析，限流与反馈指纹共用）、安全加固（docs 开关 / ALLOWED_HOSTS / 安全响应头 / 强口令）、最小运维（backup.sh 无锁备份 + --stop-app 停机窗口 / 探针 / 日志）、GitHub Actions 门禁（pytest + 6 冒烟 + k6 10k + rate_limit 单独实例）。*代码实施完成 + 本机裸机验证（2026-08-29，269 测试全绿），实施计划与验收结果见 [docs/P6_PLAN.md](P6_PLAN.md)；Docker 全栈服务器实跑与 CI 实跑待回填。*
